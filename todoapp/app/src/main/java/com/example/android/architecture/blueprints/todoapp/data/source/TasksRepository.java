...
public class TasksRepository implements TasksDataSource {
        ...
        if (mCacheIsDirty) {
            ...
            return Flowable.concat(localTasks, remoteTasks)
                    .filter(new Predicate<List<Task>>() {
                        @Override
                        public boolean test(@io.reactivex.annotations.NonNull List<Task> tasks) throws Exception {
    ...
    private Flowable<List<Task>> getAndCacheLocalTasks() {
        return mTasksLocalDataSource.getTasks()
                .flatMap(new Function<List<Task>, Publisher<List<Task>>>() {
                    @Override
                    public Publisher<List<Task>> apply(@io.reactivex.annotations.NonNull List<Task> tasks) throws Exception {
                        return Flowable.fromIterable(tasks).doOnNext(new Consumer<Task>() {
                            @Override
    ...
    public Flowable<Optional<Task>> getTask(@NonNull final String taskId) {
        ...
        Flowable<Optional<Task>> remoteTask = mTasksRemoteDataSource
                .getTask(taskId)
                .doOnNext(new Consumer<Optional<Task>>() {
                    @Override
                    public void accept(Optional<Task> taskOptional) throws Exception {
