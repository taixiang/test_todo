...
public class TasksRepositoryTest {
    ...
    public void getTask_whenDataNotLocal_fails() {
        ...
        testSubscriber.assertValue(Optional.<Task>absent());
    }
    ...
    private void setTasksNotAvailable(TasksDataSource dataSource) {
        when(dataSource.getTasks()).thenReturn(Flowable.<List<Task>>just(Collections.<Task>emptyList()));
    }

    private void setTasksAvailable(TasksDataSource dataSource, List<Task> tasks) {
        // don't allow the data sources to complete.
        when(dataSource.getTasks()).thenReturn(Flowable.just(tasks).concatWith(Flowable.<List<Task>>never()));
    }

    private void setTaskNotAvailable(TasksDataSource dataSource, String taskId) {
        when(dataSource.getTask(eq(taskId))).thenReturn(Flowable.<Optional<Task>>just(Optional.<Task>absent()));
    }

    private void setTaskAvailable(TasksDataSource dataSource, Optional<Task> taskOptional) {
        when(dataSource.getTask(eq(taskOptional.get().getId()))).thenReturn(Flowable.just(taskOptional).concatWith(Flowable.<Optional<Task>>never()));
    }
}
